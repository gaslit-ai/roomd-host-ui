...
const McpAppPartInner: FC<InnerProps> = ({
  ...
  // when a non-MCP frontend tool returns raw structured data. When passing through, best-effort populate
  ...
  const toolResult = useStableJson(
    useMemo<CallToolResult | undefined>(() => {
      ...
      // Non-MCP frontend tools can return raw structured data. Wrap it
      // into a CallToolResult so the widget receives the MCP tool-result
      // envelope.
  ...
  // --- Server → View relay -------------------------------------------------
  // `McpClientProvider` owns the single SDK notification handler for each
  // list_changed method and fans out here. HostAppRenderer then forwards the
  // invalidation to its one View via AppBridge's `send*ListChanged` helpers.
  // To force-notify the View from a non-MCP source, call
  // `appRef.current?.sendToolListChanged()` (etc.) directly.
  ...
  // Pre-read the UI resource so we can extract `_meta.ui.{csp,permissions,domain}`.
  // The CSP half is forwarded to the sandbox page via URL params. This is a
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        setResourceSandbox(readMcpUiResourceSandbox(read));
      } catch (err) {
        if (cancelled) return;
        log.warn(
  ...
  const resourceDomain =
    resourceSandbox === "loading" ? null : resourceSandbox.domain;

  const sandboxResult = useMemo(() => {
    if (resourceCsp === "loading") return { url: null };
    return buildMcpSandboxUrl({
      csp: resourceCsp,
      resourceDomain,
    });
  }, [resourceCsp, resourceDomain]);
